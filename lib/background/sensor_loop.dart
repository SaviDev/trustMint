...
const Duration _kSampleInterval = Duration(milliseconds: 500); // 2 Hz
...
Future<SensorLoop> createSensorLoop(String sessionId) async {
  final storage = SecureStorage();
  final userId = await storage.getUserId() ?? 'unknown';
  final db = dbInstance;
  return SensorLoop(db: db, userId: userId, sessionId: sessionId);
}
